"""MACD Crossover strategy — MACD line / signal line crossovers."""
...
def _v1_signals(symbol: str, closes: list[float]) -> List[Signal]:
    """Return buy/sell on MACD crossover.
    ...
    if prev_macd < prev_sig and curr_macd > curr_sig:
        confidence = max(0.4, min(0.85, abs(histogram) * 50 + 0.4))
        return [Signal(
            symbol=symbol,
            side="buy",
            confidence=confidence,
            size_hint=confidence,
            reason=f"MACD bullish crossover: histogram {histogram:.4f}",
    ...
    if prev_macd > prev_sig and curr_macd < curr_sig:
        ...
        return [Signal(
            ...
            confidence=confidence,
            size_hint=confidence,
            reason=f"MACD bearish crossover: histogram {histogram:.4f}",
            strategy=STRATEGY_NAME,
        )]

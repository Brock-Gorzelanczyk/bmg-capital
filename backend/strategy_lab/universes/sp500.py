"""S&P 500 universe for portfolio-rank bots.

Hardcoded snapshot of current S&P 500 constituents. Ships with staleness
acceptable per Phase 2 spec: an ~11-month-old list is close enough for
a momentum + quality dry-run since constituent turnover is under 5% per
year and the top-decile signal survives that noise. A nightly refresh
from IVV or SPY holdings feed is a Phase 3 follow-up.

Names deliberately capped near 500. Any additions or drops between
snapshots and today just show up as "no fundamentals data" during
rebalance and the runner skips the ticker with a warning.
"""
...
_SP500: list[str] = [
    ...
    "CMS", "CNC", "CNP", "COF", "COIN", "COO", "COP", "COR", "COST", "CPB",
    ...
    # 2026-07-07 dropped delisted: HOLX, JNPR (verified inactive on Alpaca)
    "HD", "HES", "HIG", "HII", "HLT", "HON", "HPE", "HPQ", "HRL", "HSIC",
    "HST", "HSY", "HUBB", "HUM", "HWM", "IBKR", "IBM", "ICE", "IDXX",
    "IEX", "IFF", "INCY", "INTC", "INTU", "INVH", "IP", "IPG", "IQV", "IR",
    "IRM", "ISRG", "IT", "ITW", "IVZ", "J", "JBHT", "JBL", "JCI", "JKHY",
    "JNJ", "JPM", "K", "KDP", "KEY", "KEYS", "KHC", "KIM", "KKR",
    ...
    "WMT", "WRB", "WST", "WTW", "WY", "WYNN", "XEL", "XOM", "XRAY", "XYL",
    "YUM", "ZBH", "ZBRA", "ZTS",
]


def sp500() -> list[str]:
    """Return current S&P 500 tickers (hardcoded snapshot, ~500 names)."""
    return list(_SP500)

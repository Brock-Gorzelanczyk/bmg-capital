"""Alpaca LIVE trading adapter — US equities.
...
class LiveStocksAdapter(BrokerAdapter):
    """BrokerAdapter backed by the Alpaca LIVE endpoint for US stocks.
    ...
    def submit_order(self, symbol: str, qty: float, side: str) -> dict:
        ...
        logger.warning("[LIVE-STOCKS] REAL ORDER submitted: %s %s x%.4f → id=%s", side, symbol, qty, data.get("id"))

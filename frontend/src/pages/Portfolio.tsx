...
function Skeleton({ className }: { className?: string }) {
  return <div className={cn("animate-pulse bg-[#1E293B] rounded", className)} />;
}
...
function PerfChart({ snapshots, spyBars, spyDates, loading }: PerfChartProps) {
  ...
  if (snapshots.length < 2) {
    return (
      <div className="flex items-center justify-center h-[200px] text-[#475569] text-sm">
  ...
  return (
    <div className="relative w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-[200px]" preserveAspectRatio="none">
        ...
        {/* Baseline at 100 */}
        {minY < 100 && maxY > 100 && (
          <line x1={PAD.left} y1={toY(100)} x2={W - PAD.right} y2={toY(100)} stroke="#334155" strokeWidth={1} strokeDasharray="4,4" />
        )}
        ...
        {/* Portfolio line */}
        <path d={portPath} fill="none" stroke="#3B82F6" strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
        ...
        <text x={W - PAD.right + 4} y={toY(portNorm[portNorm.length - 1])} dominantBaseline="middle"
          fill="#3B82F6" fontSize={10} fontFamily="JetBrains Mono, monospace">
      ...
      <div className="flex items-center gap-4 mt-1 px-1">
        <span className="flex items-center gap-1.5 text-xs text-[#94A3B8]">
          <span className="inline-block w-4 h-0.5 bg-[#3B82F6]" /> Portfolio
        </span>
        {spyPath && (
...
function PositionDrawer({ position, onClose, onTrade }: DrawerProps) {
  ...
  const pnlPct = position.unrealized_pnl_pct * 100;
  const dayPct = position.day_pnl_pct * 100;
  ...
  return (
    <>
      ...
      />
      {/* Drawer panel */}
      <div className="fixed right-0 top-0 bottom-0 w-80 bg-[#0F172A] border-l border-[#1E293B] z-50 flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
          ...
          {TICKER_NAMES[position.symbol] && (
            <p className="text-xs text-[#475569]">{TICKER_NAMES[position.symbol]}</p>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="bg-[#1E293B] rounded-lg px-3 py-2.5">
                <div className="text-[10px] text-[#475569] uppercase tracking-wider">{label}</div>
                <div className="font-mono text-[#F8FAFC] text-sm mt-0.5">{value}</div>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs text-[#475569] uppercase tracking-wider">Total Return</span>
...
function HoldingsTab({
  ...
  function SortHeader({ col, children, className }: { col: HoldingsSort; children: React.ReactNode; className?: string }) {
    ...
    return (
      <th
        className={cn("py-2.5 text-[10px] uppercase tracking-wider cursor-pointer select-none transition-colors", active ? "text-[#F8FAFC]" : "text-[#475569] hover:text-[#94A3B8]", className)}
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
        <div className="w-16 h-16 rounded-full bg-[#1E293B] flex items-center justify-center">
          <Layers size={28} className="text-[#475569]" />
        </div>
        <div>
          <p className="text-[#F8FAFC] font-semibold">No positions yet</p>
          <p className="text-[#475569] text-sm mt-1">Start trading to build your portfolio</p>
        </div>
        <button
          onClick={onSeedDemo}
          className="flex items-center gap-2 bg-[#3B82F6] hover:bg-[#2563EB] text-white font-semibold text-sm px-4 py-2 rounded-lg transition-colors cursor-pointer"
        >
...
function AllocationTab({ positions }: { positions: PaperPosition[] }) {
  ...
  return (
    <div className="px-4 py-5 space-y-8">
      {/* By Symbol */}
      <div>
        <h3 className="text-xs font-medium text-[#475569] uppercase tracking-wider mb-4">By Symbol</h3>
            ...
            {symbolSlices.map((s) => {
              ...
              return (
                <div key={s.label} className="flex items-center gap-2.5">
                  ...
                  <span className="font-mono font-bold text-[#F8FAFC] text-sm w-12 shrink-0">{s.label}</span>
                  <div className="flex-1 h-1.5 bg-[#1E293B] rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: s.color }} />
                  </div>
                  <span className="font-mono text-xs text-[#94A3B8] w-12 text-right">{pct.toFixed(1)}%</span>
                  <span className="font-mono text-xs text-[#475569] w-24 text-right">{formatCurrency(s.value)}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* By Sector */}
      <div>
        <h3 className="text-xs font-medium text-[#475569] uppercase tracking-wider mb-4">By Sector</h3>
        <div className="flex gap-8 items-center flex-wrap">
          ...
          <div className="flex-1 min-w-[200px] space-y-2">
            {sectorSlices.map((s) => {
              ...
              return (
                <div key={s.label} className="flex items-center gap-2.5">
                  <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: s.color }} />
                  <span className="font-medium text-[#F8FAFC] text-sm w-32 shrink-0">{s.label}</span>
                  <div className="flex-1 h-1.5 bg-[#1E293B] rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: s.color }} />
                  </div>
                  <span className="font-mono text-xs text-[#94A3B8] w-12 text-right">{pct.toFixed(1)}%</span>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map(({ pos, metrics }) => (
            <tr key={pos.id} className="border-b border-[#1E293B]/50 hover:bg-[#1E293B]/30 transition-colors">
              <td className="px-4 py-2.5">
                <div className="flex items-center gap-2">
              ...
              {metrics ? (
                <>
                  <td className={cn("px-3 py-2.5 text-right font-medium font-mono tabular-nums", metrics.annReturn >= 0 ? "text-[#22C55E]" : "text-[#EF4444]")}>
                    {metrics.annReturn >= 0 ? "+" : ""}{formatPercent(metrics.annReturn * 100)}
                  </td>
                  <td className="px-3 py-2.5 text-right text-[#94A3B8] font-mono tabular-nums">{formatPercent(metrics.annVol * 100)}</td>
                  <td className={cn("px-3 py-2.5 text-right font-medium font-mono tabular-nums", metrics.sharpe >= 1 ? "text-[#22C55E]" : metrics.sharpe >= 0 ? "text-[#94A3B8]" : "text-[#EF4444]")}>
                    {metrics.sharpe.toFixed(2)}
                  </td>
                  <td className="px-3 py-2.5 text-right text-[#EF4444] font-mono tabular-nums">-{formatPercent(metrics.maxDD * 100)}</td>
                  <td className="px-4 py-2.5 text-right text-[#94A3B8] font-mono tabular-nums">
                    {metrics.beta != null ? metrics.beta.toFixed(2) : "—"}
                  </td>
                </>
              ) : (
                <td colSpan={5} className="px-3 py-2.5 text-center text-[#475569] text-xs">Insufficient data</td>
              )}
            </tr>
          ))}
        </tbody>
      </table>
      <div className="px-4 py-2.5 text-[10px] text-[#475569] border-t border-[#1E293B]">
...
function RecentActivity({ transactions }: { transactions: PaperTransaction[] }) {
  ...
  return (
    <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl overflow-hidden">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-4 py-3 hover:bg-[#1E293B]/30 transition-colors cursor-pointer"
      >
        <span className="flex items-center gap-2 text-sm font-medium text-[#94A3B8]">
          <Clock size={14} /> Recent Activity
          {recent.length > 0 && (
            <span className="bg-[#1E293B] text-[#475569] text-[10px] font-mono rounded-full px-1.5 py-0.5">{recent.length}</span>
          )}
        </span>
      ...
      {open && (
        <div className="border-t border-[#1E293B]">
          {recent.length === 0 ? (
            <div className="px-4 py-6 text-center text-[#475569] text-sm">No filled orders yet</div>
          ) : (
              ...
              {recent.map((tx) => (
                <div key={tx.id} className="flex items-center justify-between px-4 py-2.5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-[#F8FAFC] text-sm">{tx.symbol}</span>
                        <span className={cn("text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded", tx.side === "buy" ? "bg-[#22C55E]/10 text-[#22C55E]" : "bg-[#EF4444]/10 text-[#EF4444]")}>
                          {tx.side}
                        </span>
                      </div>
                      <div className="text-xs text-[#475569]">
...
function HeroBar({ account, loading }: { account: PaperAccount | undefined; loading: boolean }) {
  if (loading) {
    return (
      <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-5 grid grid-cols-1 sm:grid-cols-3 gap-5">
  ...
  const dayPct = account.day_pnl_pct * 100;
  const totalPct = account.total_pnl_pct * 100;
  ...
  return (
    <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-5">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {/* Equity */}
        <div>
          <div className="flex items-center gap-1.5 text-xs text-[#475569] uppercase tracking-wider mb-1">
            <TrendingUp size={11} /> Portfolio Value
          </div>
          <div className="text-4xl font-bold text-[#F8FAFC] font-mono tracking-tight">
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            <Wallet size={11} className="text-[#475569]" />
        ...
        <div>
          ...
          <div className="mt-1.5 flex items-center gap-2">
            ...
            <span className="text-xs text-[#475569]">vs yesterday</span>
          </div>
        </div>
    ...
    <div className="space-y-4 pb-20 md:pb-8">
      ...
      <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-[#94A3B8]">Performance vs SPY</h3>
          {account && (
            <span className="text-xs text-[#475569] font-mono">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={cn(
                ...
                activeTab === tab.key
                  ? "text-[#F8FAFC] border-b-2 border-[#3B82F6] -mb-px"
            ...
            >
              ...
              {tab.key === "holdings" && positions.length > 0 && (
                <span className="bg-[#1E293B] text-[#94A3B8] text-[10px] font-mono rounded-full px-1.5 ml-0.5">
                  {positions.length}
                </span>
              )}
            </button>

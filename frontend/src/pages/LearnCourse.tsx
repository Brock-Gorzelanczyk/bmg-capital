...
const TRACK_ACCENT: Record<string, string> = {
  emerald: "text-[#22C55E] bg-[#22C55E]/10 border-emerald-500/20",
  blue: "text-blue-400 bg-blue-500/10 border-blue-500/20",
  violet: "text-violet-400 bg-violet-500/10 border-violet-500/20",
  amber: "text-amber-400 bg-amber-500/10 border-amber-500/20",
  rose: "text-[#EF4444] bg-rose-500/10 border-rose-500/20",
...
export default function LearnCourse() {
  ...
  if (!track) {
    return (
      <div className="flex items-center justify-center h-64 text-[#475569]">
        Track not found. <button className="ml-2 text-white underline" onClick={() => navigate("/learn")}>Back to Learn</button>
      </div>
    );
  }
  ...
  const accent = TRACK_ACCENT[track.colorClass] ?? "text-[#94A3B8] bg-[#1E293B] border-[#334155]";
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-[#475569] hover:text-[#F8FAFC] text-sm transition-colors"
      >
      ...
      <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#94A3B8] text-sm leading-relaxed">{track.description}</p>
              ...
              <div className="p-4 border-b border-[#1E293B]">
                <div className="flex items-center gap-3">
                  {courseComplete ? (
                    ...
                    <Lock size={18} className="text-[#475569] shrink-0" />
                  ) : (
                    <BookOpen size={18} className="text-[#475569] shrink-0" />
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-white text-sm">{course.title}</p>
                    <p className="text-xs text-[#475569] truncate">{course.description}</p>
                  </div>
                  {!isLocked && !courseComplete && firstIncomplete && (
                    <button
                      onClick={() => navigate(`/learn/lesson/${firstIncomplete.id}`)}
                      className="shrink-0 flex items-center gap-1 text-xs font-semibold text-white hover:text-[#22C55E] transition-colors"
                    >
                  ...
                  {courseComplete && (
                    <span className="shrink-0 text-xs text-[#22C55E] font-semibold">Complete ✓</span>
                  )}
                </div>
                {coursePct > 0 && (
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        !isLocked && "cursor-pointer hover:bg-[#1E293B]/50",
                        isLocked && "opacity-50"
                      )}
                    >
                      <div className={cn(
                        "w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-sm",
                        done ? "bg-[#22C55E]/20 text-[#22C55E]" : "bg-[#1E293B] text-[#475569]"
                      )}>
                        {done ? <CheckCircle2 size={14} /> : <span className="text-[11px] font-bold">{lessonIdx + 1}</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={cn("text-sm font-medium truncate", done ? "text-[#94A3B8]" : "text-white")}>
                          {lesson.title}
                        </p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[10px] text-[#475569] capitalize">{lesson.type}</span>
                          <span className="text-[10px] text-[#475569]">·</span>
                          <Clock size={10} className="text-[#475569]" />
                          <span className="text-[10px] text-[#475569]">{lesson.durationMin}m</span>
                          {lesson.type === "challenge" && <FlaskConical size={10} className="text-amber-500" />}
                        </div>
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        <Zap size={11} className={done ? "text-[#475569]" : "text-amber-400"} />
                        <span className={cn("text-xs font-semibold", done ? "text-[#475569]" : "text-[#94A3B8]")}>
                          {lesson.xp}
                        </span>
                      </div>
                    </div>

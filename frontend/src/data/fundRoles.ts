...
export const FUND_ROLES: FundRole[] = [
  ...
  },
  {
    ...
    status: "offline",
    description:
      "Manages the candidate pipeline: evaluates BacktestRun and WfaRun results against gates, advances candidates through BACKTEST_DONE → WFA_QUEUED → WFA_DONE → SHADOW_PAPER, retires failures, and posts status to Discord. Shadow candidates ≥63d surface in the Attention panel for CIO promotion.",
    authority: {
      tierA: [
        "Pipeline gate evaluation (Sharpe, PBO, WFE, drawdown)",
        "State transitions: BACKTEST_DONE → WFA_QUEUED, WFA_DONE → SHADOW_PAPER or RETIRED",
        "Candidate state history writes",
        "Heartbeat and Discord posts",
      ],
      tierB: [
        "SHADOW_PAPER → PROMOTED (CIO must approve via Attention panel)",
      ],
      tierC: [],
    },
    reportsTo: "Portfolio Manager (Queen)",
    inputsFrom: [
      "strategy_candidates table",
      "backtest_runs table",
      "wfa_runs table",
    ],
    outputsTo: [
      "candidate_state_history — transition audit log",
      "Discord #quant-signals",
      "agent_messages — heartbeat channel",
    ],
    discordChannels: [{ name: "#quant-signals", purpose: "Pipeline transitions and candidate digests" }],
  },
  {
    id: "macro_strategist",
